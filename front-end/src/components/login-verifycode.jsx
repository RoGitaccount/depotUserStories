...
export default function VerifyCode() {
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const { token, user } = response.data; // Supposons que la réponse contient le token et les données de l'utilisateur

      localStorage.setItem("token", token);
      localStorage.setItem("user", JSON.stringify(user)); // Sauvegarder les données utilisateur

      // Appeler la fonction login dans AuthContext pour mettre à jour le contexte
      login(user, token);
