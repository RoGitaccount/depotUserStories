...
const CartList = ({ items, onRemove }) => {
  if (!items || items.length === 0) {
    return <p className="text-center text-lg mt-6">Votre panier est vide</p>;
  }
  ...
  return (
    <div className="space-y-6">
      {items.map((item) => (
        <div
          key={item.id_produit}
          className="flex items-start gap-6 border-b border-gray-300 dark:border-gray-500 pb-6 bg-white dark:bg-gray-800 p-4 rounded transition-colors duration-300">
          ...
          <div className="flex-1 space-y-2">
            {/* Lien autour du titre aussi (optionnel) */}
            <Link to={`/produit/${item.id_produit}`}>
